...
Карта и battle-арт идут как **WebP** (отдельный набор backdrop’ов на каждый из 15 биомов), чтобы APK оставался под лимитом GitHub 100 MB и полностью офлайн.
...
- один масштабируемый сад из 15 биомов (сетка 5 × 3) и 220 грядок;
- каждый из 110 уникальных списков RSH разделён на две последовательные половины без перестановки кадров;
- первый биом содержит 10 грядок (2 × 5), остальные — по 15 (3 × 5); первые пять грядок занимают по две логические ячейки;
...
- здоровье грядки по взвешенной сумме штрихов due/new карточек и blending полной clean/negative карты;
- нажатие на открытую грядку запускает бой; открытия соседних грядок постоянны, а зарастание изменяется;
- IndexedDB-сохранение через Dexie с миграциями старых field/plot-сейвов в текущие `bed-*` без потери прогресса;
