...
Support, garden map, handwriting battle, composition dialog, stroke-order
walkthrough, cleared bed, statistics wall, achievement collection, and
achievement popup — so a design change can be judged across the whole product at once. The page provides
...
`completedWalkthroughIds` so each teaching walkthrough is acknowledged once.
