...
describe('garden model', () => {
  ...
  test('maps every character to pinyin and every available pronunciation to a local MP3', () => {
    ...
    expect(characters.find((character) => character.hanzi === '说')?.pronunciation).toEqual({ pinyin: 'shuō', audioFile: 'cmn-shuo1.mp3' })
    expect(characters.find((character) => character.hanzi === '们')?.pronunciation).toEqual({ pinyin: 'men', audioFile: 'cmn-men1.mp3' })
    expect(characters.find((character) => character.hanzi === '椅')?.pronunciation).toEqual({ pinyin: 'yǐ', audioFile: 'cmn-yi3.mp3' })
    expect(characters.find((character) => character.hanzi === '拜')?.pronunciation).toEqual({ pinyin: 'bài', audioFile: 'cmn-bai4.mp3' })
    expect(characters.find((character) => character.hanzi === '长')?.pronunciation).toEqual({ pinyin: 'cháng', audioFile: 'cmn-chang2.mp3' })
    expect(characters.find((character) => character.hanzi === '地')?.pronunciation).toEqual({ pinyin: 'dì', audioFile: 'cmn-di4.mp3' })
    expect(characters.find((character) => character.hanzi === '校')?.pronunciation).toEqual({ pinyin: 'xiào', audioFile: 'cmn-xiao4.mp3' })
  })
})
